...
why(
  ...
  proof(
    ...
    by(rule("security-incident-correlation.eye", clause(26))),
    bindings([binding("?incident", inc42)]),
    uses([
      proof(
        goal(confirmed_compromise(inc42)),
        by(rule("security-incident-correlation.eye", clause(25))),
        bindings([binding("?incident", inc42)]),
        uses([
          proof(
            goal(credential_abuse(inc42)),
            by(rule("security-incident-correlation.eye", clause(21))),
            bindings([binding("?incident", inc42), binding("?user", user_alice)]),
            uses([
              proof(
                goal(alert(inc42, suspicious_login, user_alice)),
                by(fact("security-incident-correlation.eye", clause(12)))
              ),
              proof(
                goal(privileged_user(user_alice)),
                by(fact("security-incident-correlation.eye", clause(10)))
              )
            ])
          ),
          proof(
            goal(malware_on_critical_asset(inc42)),
            by(rule("security-incident-correlation.eye", clause(22))),
            ...
            uses([
              proof(
                ...
                by(fact("security-incident-correlation.eye", clause(11)))
              ),
              proof(
                goal(alert(inc42, malware_hash, hash_redline)),
                by(fact("security-incident-correlation.eye", clause(13)))
              ),
              proof(
                goal(critical_asset(endpoint23)),
                by(rule("security-incident-correlation.eye", clause(20))),
                bindings([binding("?endpoint", endpoint23)]),
                uses([
                  proof(
                    goal(asset(endpoint23, criticality, high)),
                    by(fact("security-incident-correlation.eye", clause(6)))
                  )
                ])
              )
            ])
          ...
          proof(
            ...
            by(rule("security-incident-correlation.eye", clause(23))),
            bindings([binding("?incident", inc42), binding("?ip", ip_203_0_113_17)]),
            uses([
              proof(
                goal(alert(inc42, outbound_ip, ip_203_0_113_17)),
                by(fact("security-incident-correlation.eye", clause(14)))
              ),
              proof(
                goal(threat_intel(ip_203_0_113_17, command_and_control)),
                by(fact("security-incident-correlation.eye", clause(19)))
              )
            ])
          ),
          proof(
            goal(exploitable_endpoint(inc42)),
            by(rule("security-incident-correlation.eye", clause(24))),
            ...
            uses([
              ...
              proof(
                ...
                by(fact("security-incident-correlation.eye", clause(18)))
              )
            ])
          )
        ])
...
why(
  ...
  proof(
    ...
    by(rule("security-incident-correlation.eye", clause(27))),
    bindings([binding("?incident", inc42)]),
    uses([
      proof(
        goal(confirmed_compromise(inc42)),
...
why(
  ...
  proof(
    ...
    by(rule("security-incident-correlation.eye", clause(28))),
    bindings([binding("?incident", inc42)]),
    uses([
      proof(
        goal(confirmed_compromise(inc42)),
